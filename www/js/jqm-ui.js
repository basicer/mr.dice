...
$(document).delegate('#chartDialog','pagecreate', function() {
	$('table').hide().visualize({type: 'bar', appendKey: false, width: '200px'});
});
...
function popText(c) {
	return "<div class='popText' style='font-size:40pt; margin-left: auto; margin-right: auto; text-align: center'>" + c.getValueBox() + "</div>";
}
...
function popCalc(what) {
		...
		$("<div />").simpledialog2({
			...
			'mode' : 'button',
			showModal: false,
			forceInput: need_force_input,
			buttonPrompt : popText(c),
			   'buttons' : {
				  'OK': {
					click: function () { 
				  ...
				  'ReRoll': {
					click: function () {
						c.crunch();
						this.sdIntContent.find("p.ui-simpledialog-subtitle + div").html(popText(c));
						return false;
					},
					icon: "refresh",
					theme: "c"
...
function addRoll(roll, name) {
		var del = function() {
				deletewhat = this;
				$.mobile.changePage( "#delete", { role: "dialog", transition: quick ? "none" : "slidedown" } );

		}
		$(
				'<li data-name="' + name + '">' +
				'<a href="#" class="pop">' + name + '</a>' + 
				'<span class="ui-li-count">' + new Calculator().getClean(roll) + '</span>' + 
				'<a href="#" class="roll" data-roll=' + roll + '">Edit</a>' + 
				'</li>'
		)
...
$(document).ready(function() {
	$("#clearStorage").click(function() { 
		$(this).simpledialog({
			...
			'prompt' : "<h2>Delete Saved Data?</h2>",
			   'buttons' : {
				  'OK': { click: function() { resetRolls(); } },
				  'Cancel': { theme: "c", icon: "delete", click: function() { return true; } }
				}
	...
	*/
	...
	$("#calcKeyFX").bind("tap",function(e) { $.mobile.changePage("#functionDialog", { transition: quick ? "none" : "slide" }); return false; });
	$("#loadButton").bind("tap",function(e) { $.mobile.changePage("#presets", { transition: quick ? "none" : "slide" }); return false; });
	$("#presetsBack").bind("tap",function(e) { $.mobile.changePage("#calc", { transition: quick ? "none" : "slide", reverse: true }); return false; });
	$("#functionBack").bind("tap",function(e) { $.mobile.changePage("#calc", { transition: quick ? "none" : "slide", reverse: true }); return false; });

	$("#calcKeyC").bind("tap",function() { calculator.key("C");  return false; })
	$("#calcKeyS").bind("tap",function() {
		var v = calculator.command == null ? calculator.buffer :  calculator.command ;
		if ( v == "" ) return;
		/*
		$("#calcKeyS").simpledialog({
			'mode' : 'string',
			'prompt' : 'Save As',
			'cleanOnClose': true,
			useModal: false,
			forceInput: need_force_input,
			'useDialogForceFalse': true,
			'onClosed' : function() { blockKeys = true; },
			'onShown' : function() {  blockKeys = false; },
			'buttons' : {
				'OK': {
					click: function () {
	...
	$(".keyblock").on("tap","a", function(evt) {
		...
		if ( paddice ) {
			...
			$("#calcKey9 > span > span").html("9").css("margin-left","0px");
			paddice = false;
		} else if ( val == "d" && !paddice ) {
			paddice = true;
			$("#calcKey7 > span > span").html("20");
			$("#calcKey9 > span > span").html("<small>100</small>").css("margin-left","-5px");
		}
		return false;
	});
	...
	$("#saveroll").bind("click", function(evt) {
		...
		if ( rolls[k] !== undefined && loaded_presets ) {
			$("#rolllist").find("li[data-name='" + k + "']").remove();
		}
		rolls[k] = v
		addRoll(v,k);
