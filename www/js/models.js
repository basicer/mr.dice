...
Calculator.prototype = {
  ...
  raise: function(evt) {
    if (this[evt] != null) {
      ...
      var uvt = this;
      this.eventbuffer[evt] = setTimeout(function() {
        uvt[evt].apply(uvt);
      }, 10);
    }
  },
  ...
  getClean: function(buf) {
    ...
    buf = "" + buf;
    buf = buf.replace(/\[F:([a-zA-Z]*)\]/g, function(a, b) {
      return " " + b + " ";
    });
  ...
  calc: function(cmd, sub) {
    ...
    while (c2 != cmd) {
      c2 = cmd;
      cmd = cmd.replace(/^(.*?)\[F:([a-zA-Z]*?)\]([0-9]+)?$/, function(
        chunk,
        arg,
        name,
        opts
    ...
    cmd = cmd.replace(/(\d+|^)d(\d+)/g, function(x, a, b) {
      return calc.roll(a, b);
    });
    if (sub) {
      this.logClose();
      //this.log("-> {1}", cmd);
  ...
  embedComments: function(arg) {
    return arg.replace(/\/\*(.*?)\*\//g, function(a, m) {
      return "|*" + m + "*|";
    });
  },
  load: function(arg) {
...
Calculator.prototype.funcs = {
  ...
  close: function(arg, opt, raw) {
    ...
    for (var i in list) {
      if (parseInt(list[i]) >= opt)
        extra.push("/*O:*/" + this.roll("1d" + targets[1]));
    }
